...
import datomicJavaScala.client.api.{Datom, Datomic, DbStats}
...
class DbTest extends SetupSpec {
  ...
  "datoms" in new Setup {
    conn.db.datoms(":avet", list(read(":movie/title"))).toScala(List)
      .map(_.v.toString).sorted === threeFilms
  }
  ...
  "indexPull" in new Setup {
    ...
    // Interfaces using this facade therefore need to implement suitable adoptions
    ok
  }

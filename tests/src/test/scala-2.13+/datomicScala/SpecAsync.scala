...
trait SpecAsync extends Specification with SchemaAndData {
  ...
  def setupDevLocal(): Unit = {
    system = "dev-local"
    client = AsyncDatomic.clientDevLocal("test-datomic-client-api-scala-2.13")
  }
  ...
  class AsyncSetup extends SchemaAndData with Scope {
    ...
    def films(db: AsyncDb): Seq[String] = {
      val lazyList: LazyList[Either[CognitectAnomaly, stream.Stream[_]]] = waitFor(AsyncDatomic.q(filmQuery, db))

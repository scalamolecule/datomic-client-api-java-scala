...
class AsyncDatomicTest extends SpecAsync with AnomalyWrapper{
  ...
  : fs2.Stream[IO, Either[CognitectAnomaly, jStream[_]]] = {
    AsyncDatomic.qStream(
      Util.map(
        ...
        read(":chunk"), chunkSize.asInstanceOf[Object],
      )
    )
  }
  ...
  "q" in new AsyncSetup {
    ...
    val futureLazyChunks: Future[Stream[Either[CognitectAnomaly, jStream[_]]]] =
      AsyncDatomic.q(
        """[:find ?movie-title
          |:where [_ :movie/title ?movie-title]]""".stripMargin,
        conn.db
    ...
    waitFor(AsyncDatomic.q(
      """[:find ?movie-title
        ...
        |]""".stripMargin,
      conn.db, 1984.asInstanceOf[Object]
    )).head.toOption.get.iterator().asScala.toList === List(
      list("Repo Man")
    )
    ...
    // Chunks
    def chunksOf(chunkSize: Int): Stream[Either[CognitectAnomaly, stream.Stream[_]]] = {
      waitFor(AsyncDatomic.q(
        Util.map(
          read(":query"),
          """[:find ?movie-title
    ...
    res1(0).toOption.get.iterator().asScala.toList === List(list("Commando"))
    // Second chunk is fetched
    res1(1).toOption.get.iterator().asScala.toList === List(list("The Goonies"))
    // Third chunk is fetched
    res1(2).toOption.get.iterator().asScala.toList === List(list("Repo Man"))
    // Empty tail is also evaluated to calculate size
    res1.size === 3

    val res2 = chunksOf(2)
    res2(0).toOption.get.iterator().asScala.toList === List(list("Commando"), list("The Goonies"))
    res2(1).toOption.get.iterator().asScala.toList === List(list("Repo Man"))
    res2.size === 2

    val res3 = chunksOf(3)
    res3(0).toOption.get.iterator().asScala.toList ===

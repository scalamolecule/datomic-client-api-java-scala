...
class DbTest extends Spec {
  ...
  "history" in new Setup {
    ...
    if (isDevLocal) {
      ...
      conn.transact(list(list(read(":db/retractEntity"), retractedEid.asInstanceOf[Object])))
      films(conn.db) === threeFilms
      films(conn.db.history) === fourFilms
  ...
  "datoms" >> {
    ...
    "datoms AVET" in new Setup {
      ...
      if (system == "dev-local") {
        ...
        ).iterator().asScala.toList.size === 243
      }
...
//      val l = list(read(":movie/title"), e2, "Commando", txAfter)
  ...
  ).iterator().asScala.toList.map(_.v.toString)
  ...
  "indexRange" in new Setup {
    ...
    conn.db.indexRange(":movie/title").iterator().asScala.toList === List(
      Datom(e2, a1, "Commando", txAfter, true),
      Datom(e3, a1, "Repo Man", txAfter, true),
      Datom(e1, a1, "The Goonies", txAfter, true),
    )
    ...
    conn.db.indexRange(":movie/title", limit = -1).iterator().asScala.toList === List(
      Datom(e2, a1, "Commando", txAfter, true),
      Datom(e3, a1, "Repo Man", txAfter, true),
      Datom(e1, a1, "The Goonies", txAfter, true),
    )
  ...
  "indexPull" in new Setup {
    ...
    val firstFilmData: jMap[_, _] = Util.map(
      ...
      read(":movie/release-year"), 1985: java.lang.Integer
    )
    // But we can't

...
trait Spec extends Specification with SchemaAndData with AnomalyWrapper {
  ...
  def setupDevLocal(): Unit = {
    system = "dev-local"
    try {
      client = Datomic.clientDevLocal("test-datomic-client-api-scala-2.12")
    } catch {
      case t: Throwable =>
  ...
  class Setup extends SchemaAndData with Scope {
    ...
    if (isDevLocal) {
      ...
      Datomic.q("[:find ?e :where [?e :movie/title _]]", conn.db)
        ...
        lastTxOpt = Some(conn.transact(list(list(":db/retractEntity", eid.asInstanceOf[Object]))))
      }
    ...
    def films(db: Db): Seq[String] = {
      // Avoid scala.jdk.CollectionConverters._ to work with both scala 2.12 and 2.13
      var films = Seq.empty[String]
      Datomic.q(filmQuery, db).forEach(f => films = films :+ f.get(0).toString)
      films.sorted
    }
  }
}